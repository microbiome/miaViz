...
setMethod("plotColTree", signature = c(x = "TreeSummarizedExperiment"),
    function(x,
        ...
        .plot_row_column_tree(x, 
                              ...
                              remove.levels = remove.levels,
                              show.label = show.label,
                              show.highlights = show.highlights,
                              show.highlight.label = show.highlight.label,
                              abbr.label = abbr.label,
                              add.legend = add.legend,
                              layout = layout,
                              edge.colour.by = edge.colour.by,
                              edge.size.by = edge.size.by,
                              tip.colour.by = tip.colour.by,
                              tip.shape.by = tip.shape.by,
                              tip.size.by = tip.size.by,
                              node.colour.by = node.colour.by,
                              node.shape.by = node.shape.by,
                              node.size.by = node.size.by,
                              colour.highlights.by = colour.highlights.by,
...
.check_tree_plot_switches <- function(layout,
    ...
    if(!.is_a_bool(relabel.tree)){
        stop("'relabel.tree' must be either TRUE or FALSE.", call. = FALSE)
    }
    if(!.is_a_bool(remove.levels)){
        stop("'remove.levels' must be either TRUE or FALSE.", call. = FALSE)
    }
    ...
    if(!.is_a_bool(show.highlights)){
        if( (!is.logical(show.highlights) && !is.character(show.highlights) &&
             !is.numeric(show.highlights)) ||
            is.null(show.highlights)){
            stop("'show.label' must be either TRUE or FALSE or logical, ",
                 "integer or character ",
                 "vector. Character alues should match the label of the tree.",
                 call. = FALSE)
    ...
    if(!.is_a_bool(show.highlight.label)){
        if( (!is.logical(show.highlight.label) && !is.character(show.highlight.label) &&
...
.plot_row_column_tree <- function(x,
                                  tree.name = tree_name,
                                  tree_name = "phylo",
                                  relabel.tree = relabel_tree,
                                  relabel_tree = FALSE,
    ...
    FUN <- switch(type,
                  row = "rowTree",
                  column = "colTree")
    if(is.null(do.call(FUN,list(x = x, whichTree = tree.name)))){
        stop(FUN,"(x, tree.name) is empty.", call. = FALSE)
    }
    ...
    tree_out <- .get_object_and_trimmed_tree(
        ...
        tree.name = tree.name,
        type = type,
        relabel = relabel.tree,
        order = order.tree)
    x <- tree_out$x
    tree <- tree_out$tree
    tree_data <- .get_tree_data(tree)
    label_out <- .add_tree_node_labels(tree_data, show.label, remove.levels)
    tree_data <- label_out$df
    show.label <- label_out$show.label
    label_out <- .add_tree_highlights(tree_data, show.highlights)
    tree_data <- label_out$df
    show.highlights <- label_out$show.highlights
    label_out <- .add_tree_highlight_labels(tree_data, show.highlight.label,
                                            remove.levels)
    tree_data <- label_out$df
    show.highlight.label <- label_out$show.highlight.label
    #
    ...
    edge.size.by <- vis_out$edge.size.by
    colour.by <- vis_out$colour.by
    shape.by <- vis_out$shape.by
    size.by <- vis_out$size.by
    colour.highlights.by <- vis_out$colour.highlights.by
    show_tips <- any(!vapply(c(tip.colour.by, tip.shape.by, tip.size.by),
                             is.null, logical(1)))
    show_nodes <- any(!vapply(c(node.colour.by, node.shape.by, node.size.by),
                              is.null, logical(1)))
    #
    x <- .create_treedata_for_plotting(tree_data,
                                            tree,
                                            edge.colour.by,
                                            edge.size.by,
                                            shape.by,
...
.get_object_and_trimmed_tree <- function(
    ...
    if(order){
        tree <- .order.tree(tree)
    }
    res <- list(x = x, tree = tree)
    return(res)
}
...
.add_tree_node_labels <- function(tree.data = tree_data,
                                  tree_data,
                                  show.label = show_label,
                                  show_label,
                                  remove.levels = remove_levels,
    ...
    if(!is.logical(show.label) || length(show.label) > 1L) {
        if(is.character(show.label) && 
           length(show.label) == nrow(tree_data)) {
            tree_data <- tree_data %>% 
                mutate(node_label = show.label)
            show.label <- TRUE
            ...
            if(is.numeric(show.label)){
                ...
                label[tree_data$node %in% show.label] <- TRUE
                show.label <- label
            } else if(is.character(show.label)) {
                show.label <- tree_data$node_label %in% show.label
            }
            ...
            tree_data <- tree_data %>% 
                mutate(node_label = ifelse(show.label,
                                           .data$node_label,
                                           NA_character_))
            show.label <- TRUE
        }
    ...
    } else if(is.logical(show.label) && length(show.label) == 1L &&
              !show.label) {
        tree_data <- tree_data %>% 
...
#' @importFrom tidygraph activate
#' @importFrom dplyr mutate
.add_tree_highlights <- function(tree.data = tree_data, 
    tree_data, show.highlights = show_highlights, show_highlights){
    tree_data$highlight <- FALSE
    
    if(!is.logical(show.highlights) || length(show.highlights) > 1L) {
        if(is.numeric(show.highlights)){
            if(any(show.highlights != as.integer(show.highlights)) ||
               min(show.highlights) < 1 ||
               max(show.highlights) > nrow(tree_data)){
            ...
            label <- rep(FALSE, nrow(tree_data))
            label[tree_data$node %in% show.highlights] <- TRUE
            show.highlights <- label
        } else if(is.character(show.highlights)) {
            show.highlights <- tree_data$label %in% show.highlights
        }
        if(is.logical(show.highlights) &&
           length(show.highlights) != nrow(tree_data)){
            stop("If 'show.highlights' is logical, it must have the length as ",
                 "nodes are in the graph.",
                 call. = FALSE)
        }
        tree_data <- tree_data %>% 
            mutate(highlight = show.highlights)
        show.highlights <- TRUE
        if(!any(tree_data %>% pull("highlight"))){
            show.highlights <- FALSE
            warning("No highlights to plot.", call. = FALSE)
        }
    } else if(is.logical(show.highlights) && length(show.highlights) == 1L &&
              show.highlights){
...
#' @importFrom tidygraph activate
#' @importFrom dplyr mutate
.add_tree_highlight_labels <- function(tree.data = tree_data,
                                       tree_data,
                                       show.highlight.label = show_highlight_label,
                                       show_highlight_label,
                                       remove.levels = remove_levels,
    ...
    if(!is.logical(show.highlight.label) || 
       length(show.highlight.label) > 1L) {
        if(is.character(show.highlight.label) && 
           length(show.highlight.label) == nrow(tree_data)) {
            tree_data <- tree_data %>% 
                mutate(highlight_label = show.highlight.label)
            show.highlight.label <- TRUE
            ...
            if(is.numeric(show.highlight.label)){
                if(any(show.highlight.label != as.integer(show.highlight.label)) ||
                   min(show.highlight.label) < 1 ||
                   max(show.highlight.label) > nrow(tree_data)){
                ...
                label <- rep(FALSE, nrow(tree_data))
                label[tree_data$node %in% show.highlight.label] <- TRUE
                show.highlight.label <- label
            } else if(is.character(show.highlight.label)) {
                show.highlight.label <- 
                    tree_data$highlight_label %in% show.highlight.label
            }
            if(is.logical(show.highlight.label) &&
               length(show.highlight.label) != nrow(tree_data)){
            ...
            tree_data <- tree_data %>% 
                mutate(highlight_label = ifelse(show.highlight.label &
                                                    tree_data$highlight,
    ...
    } else if(is.logical(show.highlight.label) &&
              length(show.highlight.label) == 1L &&
              !show.highlight.label){
        tree_data <- tree_data %>%
...
TIP_VARIABLES <- c("tip.colour.by", "tip.shape.by", "tip.size.by")
NODE_VARIABLES <- c("node.colour.by", "node.shape.by", "node.size.by")
...
.merge_tip_node_tree_data <- function(tree_data){
    ...
    #
    cn <- colnames(tree_data)
    if(all(c("tip.colour.by","node.colour.by") %in% cn)){
        colour.by <- c(tree_data$node.colour.by[!is_leaf_o],
                       tree_data$tip.colour.by[is_leaf_o])
    } else if("tip.colour.by" %in% cn) {
        colour.by <- tree_data$tip.colour.by
    } else if("node.colour.by" %in% cn) {
        colour.by <- tree_data$node.colour.by
    }
    if(all(c("tip.shape.by","node.shape.by") %in% cn)){
        shape.by <- c(tree_data$node.shape.by[!is_leaf_o],
                      tree_data$tip.shape.by[is_leaf_o])
    } else if("tip.shape.by" %in% cn) {
        shape.by <- tree_data$tip.shape.by
    } else if("node.shape.by" %in% cn) {
        shape.by <- tree_data$node.shape.by
    }
    if(all(c("tip.size.by","node.size.by") %in% cn)){
        size.by <- c(tree_data$node.size.by[!is_leaf_o],
                     tree_data$tip.size.by[is_leaf_o])
    } else if("tip.size.by" %in% cn) {
        size.by <- tree_data$tip.size.by
    } else if("node.size.by" %in% cn) {
        size.by <- tree_data$node.size.by
    }
    #
    tree_data <- tree_data[,cn[!grepl("tip_|node_",cn) | cn == "node_label"]]
    tree_data$colour.by <- colour.by
    tree_data$shape.by <- shape.by
    tree_data$size.by <- size.by
...
# due to a bug in ggtree/tidytree the treedata x needs to be constructed
# in a separate step
# 
# also there is some data wrangling needed
#' @importFrom tidytree as.treedata
.create_treedata_for_plotting <- function(tree.data = tree_data,
                                          tree_data,
                                          tree,
                                          edge.colour.by = edge_colour_by,
                                          edge_colour_by,
    ...
    if (!is.null(edge.colour.by) && 
        anyNA(tree_data$edge.colour.by) &&
        !is.numeric(tree_data$edge.colour.by)) {
        tree_data <- groupOTU(tree_data, 
                              split(tree_data$node, tree_data$edge.colour.by),
                              group_name = "group")
        f_zero <- tree_data$group != 0
        f_zero <- f_zero[!is.na(f_zero)]
        tree_data$edge.colour.by[f_zero] <- 
    ...
    tree_data <- .na_replace_from_plot_data(tree_data,
                                            ...
                                            shape.by,
                                            size.by)
    x <- tidytree::as.treedata(tree_data)
...
.get_hightlight_args <- function(nodes, colour.highlights.by = colour_highlights_by, 
    colour_highlights_by = NULL){
    aes_args <- list()
    aes_args$subset <- paste0("node %in% c(",paste(nodes, collapse = ","),")")
    aes_args$extendto <- ~highlight_extendto
    if(!is.null(colour.highlights.by)) {
        aes_args$fill <- ~colour.highlights.by
    }
...
.plot_tree_plot_highlights <- function(plot.out = plot_out,
                                       ...
                                       colour.highlights.by = colour_highlights_by,
                                       colour_highlights_by,
                                       highlight.fone.size = highlight_font_size,
                                       highlight_font_size){
    ...
    if(show.highlights && nrow(plot_out$data) > 0L){
...
.plot_tree_edges <- function(plot.out = plot_out,
                             ...
                             layout){
    # assemble arg list
    edge_out <- .get_edge_args(edge.colour.by,
                               edge.size.by,
                               alpha = line_alpha,
                               size = line_width,
                               layout = layout)
...
.plot_tree_node_points <- function(plot.out = plot_out,
    ...
    if(any(c(show_tips,show_nodes)) && !is.null(size.by)){
        if(is.numeric(plot_out$data$size.by)){
