...
#'   \code{\link[ggtree]{ggtree}} for details.
#'
...
#' @rdname plotTree
#' @export
setMethod("plotRowTree",
    signature = c(x = "TreeSummarizedExperiment"),
    function(x, tree.name = "phylo", ...) {
        p <- .plot_row_column_tree(
            x,
            tree.name = tree.name, type = "row", ...
        )
        return(p)
    }
)
...
.plot_row_column_tree <- function(x, tree.name, type, ...) {
    ...
    x <- .get_object_and_trimmed_tree(
        x,
        tree.name = tree.name, type = type, ...
    )
    # Get tree from TreeSE
    tree_FUN <- switch(type,
        row = rowTree,
        column = colTree,
        stop(".")
    )
...
.get_object_and_trimmed_tree <- function(x,
                                         tree.name = "phylo",
                                         type = c("row", "column"),
    ...
    check_FUN <- switch(type,
        row = .check_rowTree_present,
        column = .check_colTree_present
    )
    ...
    links_FUN <- switch(type,
        row = rowLinks,
        column = colLinks,
        stop(".")
    )
    dimnames_FUN <- switch(type,
        row = rownames,
        column = colnames,
        stop(".")
    )
    add_names_FUN <- switch(type,
        row = `rownames<-`,
        column = `colnames<-`,
        stop(".")
    )
    ...
    if (anyDuplicated(tree_data$label[-m])) {
        warning(
            "Tree includes ununique nodes. Making them unique.",
            call. = FALSE
        )
...
.add_tree_node_labels <- function(df, show.label = show_label, show_label = FALSE,
...
#' @importFrom tidygraph activate
#' @importFrom dplyr mutate
.add_tree_highlights <- function(df, show.highlights = show_highlights, show_highlights = FALSE, ...) {
...
#' @importFrom tidygraph activate
#' @importFrom dplyr mutate
.add_tree_highlight_labels <- function(df, show.highlight.label = show_highlight_label,
    ...
    if (any(df[["highlight"]]) && "label" %in% colnames(df)) {
        ...
        if (is.character(show.highlight.label)) {
            df[
                !df[["highlight_label"]] %in% show.highlight.label,
                "highlight_label"
            ] <- NA_character_
        }
    }
...
.incorporate_tree_vis <- function(df, x, type,
...
.retrieve_variable <- function(x, var, type, assay.type = "counts", ...) {
    name_FUN <- switch(type,
        row = colnames,
        column = rownames
    )
    rowdata_FUN <- switch(type,
        row = rowData,
        column = colData
    )
...
# shape and size.
.combine_tree_point_formatting <- function(df,
                                           tip_colour_by = NULL,
                                           node_colour_by = NULL,
                                           tip_shape_by = NULL,
                                           node_shape_by = NULL,
    ...
    if (any(node_var %in% colnames(df))) {
        df_node <- df[
            !isTip(df, df[["node"]]), colnames(df) %in% node_var,
            drop = FALSE
        ]
...
.tree_plotter <- function(df,
    ...
    plot_out <- ggtree(
        df,
        ladderize = !order.tree, layout = layout,
        branch.length = branch.length, ...
    )
    # add highlights
...
.add_label_abbreviations <- function(plot_out, label_col, subset = NULL) {
...
.get_cladelab_args <- function(nodes, layout, highlight_font_size) {
    aes_args <- list()
    aes_args$subset <- paste0("node %in% c(", paste(nodes, collapse = ","), ")")
    aes_args$node <- ~node
...
.plot_tree_plot_highlights <- function(plot_out, layout, show_highlights, show_highlight_label, abbr_label,
...
.plot_tree_edges <- function(plot_out, edge_colour_by, edge_size_by, line_alpha, line_width,
                             line_width_range, layout) {
    # assemble arg list
    edge_out <- .get_edge_args(
        edge_colour_by, edge_size_by,
        alpha = line_alpha, size = line_width,
        layout = layout
    )
    plot_out <- plot_out +
...
.plot_tree_node_points <- function(plot_out, show_tips, show_nodes, colour_by, shape_by, size_by,
    ...
    if (!is.null(colour_by)) {
        plot_out <- .resolve_plot_colours(
            plot_out, plot_out$data$colour_by, colour_by,
            fill = point_out$fill,
            na.translate = FALSE
        )
    }
...
.plot_tree_node_labels <- function(plot_out, show_label, abbr_label, label_font_size) {
    if (show_label) {
        data <- plot_out$data
