...
#'   \code{?\link[scater]{retrieveFeatureInfo}} or \code{?\link[scater]{retrieveCellInfo}}. Must
...
.tile_plotter <- function(data,
    ...
    plot_out <- .resolve_plot_colours(
        plot_out, data$colour_by, ylab,
        fill = TRUE, na.value = na.value
    )
