...
test_that("Test getNeatOrder function", {
    ...
    result <- getNeatOrder(neatsort_matrix, dimensions = c(1, 2), centering.method = "mean")
    expected <- c("Sample4", "Sample2", "Sample3", "Sample1")
    expect_equal(result, expected)
    
    # Test with valid inputs, decreasing parameter = TRUE
    result <- getNeatOrder(neatsort_matrix, dimensions = c(1, 2), centering.method = "mean", decreasing = TRUE)
    expected <- c("Sample1", "Sample3", "Sample2", "Sample4")
    expect_equal(result, expected)
    
    # Test with dimensions col names
    result <- getNeatOrder(neatsort_matrix, dimensions = c("PC1", "PC2"), centering.method = "mean")
    expected <- c("Sample4", "Sample2", "Sample3", "Sample1")
    expect_equal(result, expected)
})
...
test_that("Test check_args method", {
    ...
    expect_error(.check_args(check_args_matrix),
                 "argument \"dimensions\" is missing, with no default")
    expect_error(.check_args(check_args_matrix, c(1, 3, 4), c(1, 3)),
                 "argument \"centering.method\" is missing, with no default")
    expect_error(.check_args(check_args_matrix, c(1, 3, 4), c(1, 3), "mean"),
                 'argument "decreasing" is missing')
    
    # Non-matrix input
    expect_error(.check_args(as.data.frame(check_args_matrix), NULL, c(1, 2), "mean", FALSE),
                 "Input data must be a matrix.")
    
    # Test for empty matrix
    empty_matrix <- matrix(numeric(0), nrow = 0, ncol = 0)
    expect_error(.check_args(empty_matrix, NULL, c(1, 2), "mean", FALSE),
                 "No data to plot. Matrix must have at least one row and one column.")
    
    # Test for invalid dimensions
    expect_error(.check_args(check_args_matrix, NULL, c(1, 6), "mean", FALSE),
                 "dimensions refer to columns that do not exist in the data.")
    expect_error(.check_args(check_args_matrix, NULL, c("PC6"), "mean", FALSE),
                 "dimensions refer to column names that do not exist in the data.")
    expect_error(.check_args(check_args_matrix, NULL, list("PC6"), "mean", FALSE),
                 "dimensions must be a vector of column indices or names.")
    expect_error(.check_args(check_args_matrix, NULL, c(1, 2, 3), "mean", FALSE),
                 "Exactly two dimensions must be specified.")
    
    # Test for invalid centering_method
    expect_error(.check_args(check_args_matrix, NULL, c(1, 2), "invalid_method", FALSE),
                 "'arg' should be one of “mean”, “median”, “none”")
    
    # Test for invalid sorting order
    expect_error(.check_args(check_args_matrix, NULL, c(1, 2), "mean", "invalid_order"),
    ...
    non_unique_row_matrix <- matrix(1:20, nrow = 4, ncol = 5,
                                    dimnames = list(c("Sample1", "Sample2", "Sample1", "Sample4"),
                                                    c("PC1", "PC2", "PC3", "PC4", "PC5")))
    expect_error(.check_args(non_unique_row_matrix, NULL, c(1, 2), "mean", FALSE),
    ...
    expect_error(.check_args(non_unique_col_matrix, NULL, c(1, 2), "mean", FALSE),
...
test_that("Test get_sorted_rownames method", {
    ...
    expect_error(.get_sorted_rownames(theta_values),
                 'argument "decreasing" is missing')
    
    # Valid sorting in ascending order
    expected <- c("Sample2", "Sample4", "Sample1", "Sample3")
    result <- .get_sorted_rownames(theta_values, FALSE)
    expect_equal(result, expected)
    
    # Sorting in descending order
    result <- .get_sorted_rownames(theta_values, TRUE)
